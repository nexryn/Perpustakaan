...
if (isset($_POST['submit'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO anggota (nama, email, telepon, alamat) VALUES (?, ?, ?, ?)");
...
if (isset($_POST['update'])) {
    ...
    $stmt = $conn->prepare("UPDATE anggota SET nama=?, email=?, telepon=?, alamat=? WHERE id=?");
...
<head>
    ...
    <title>Kelola Anggota - Perpustakaan Yao</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        input, textarea { width: 100%; padding: 14px 18px; border: 2px solid #e0e0e0; border-radius: 12px; font-size: 14px; transition: all 0.3s ease; font-family: 'Poppins', sans-serif; }
                        ...
                        <?php if (mysqli_num_rows($result) > 0): ?>
                            ...
                            <tr>
                                ...
                                <td class="actions">
                                    ...
                                    <a href="?delete=<?php echo $row['id']; ?>" class="delete" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?')"><i class="fas fa-trash"></i> Hapus</a>
                                </td>
                            </tr>
                            <?php endwhile; ?>
                        <?php else: ?>
        ...
        <div class="card">
            ...
            <form method="POST" action="">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px;">
                    ...
                    <div class="form-group">
                        <label>Telepon</label>
                        <input type="text" name="telepon" value="<?php echo $edit_data['telepon']; ?>">
                    </div>
                </div>
                <div class="form-group">
                    <label>Alamat</label>
                    <textarea name="alamat"><?php echo $edit_data['alamat']; ?></textarea>
                </div>
                <button type="submit" name="update" class="btn btn-success"><i class="fas fa-sync-alt"></i> Update Data</button>
                <a href="anggota.php" class="btn btn-danger"><i class="fas fa-times"></i> Batal</a>
            </form>
        </div>
        <?php endif; ?>
    </div>
